...
5. Discover (Recommendation) - Based on users top movies rankings there will be a recommended list of movies that the user can watch. The recommendation list will be generated based on the top movies on the users tiered list. The list will exclude all the movies which are already ranked by the user. Each movie in the list will contain the movie name, movie banner, and movie duration. Movie detail sheets display year and 5-star rating (with partial star fill, gold color #FFD700, 14dp size, 8dp spacing) in a consistent format across the entire app.
   - Featured Movie Quote Card: The Discover page displays a daily featured movie card with an authentic movie quote. The quote system uses a three-tier fallback: (1) Wikiquote API fetches real quotes from Wikipedia's movie quote database (6-second timeout, 24-hour cache), (2) Curated database of 575+ famous movie quotes covering all major genres, franchises, and eras from classic to modern cinema, (3) Smart movie-themed inspirational messages as final fallback (never shows plot descriptions as quotes).
...
3. **Wikiquote**
   - **Purpose**: Fetch authentic movie quotes from Wikipedia's Wikiquote database for the featured movie card on Discover page. Backend service (`/api/quotes`) scrapes movie-specific Wikiquote pages using the MediaWiki API, extracts quotes from structured HTML sections, and caches results for 24 hours. Provides variety by returning random quotes from available candidates. Gracefully falls back to curated database or generic messages on API failure/timeout.
...
- **cheerio** (v1.0.0-rc.12): Fast, flexible HTML parsing and manipulation library for Node.js. Used to scrape and extract movie quotes from Wikiquote HTML pages. Provides jQuery-like API for DOM traversal and selection.
